...
Quantoz Blockchain Services provides a separate command-line tool to optionally assist with token creation independent of the Nexus platform. The intent of this tool is to support the separation of token ownership (to sit with our partners) and the operational token activities (to sit within Nexus), by creating the token outside of Nexus and then importing it into Nexus. Alternatively to this tool, ASA’s on the Algorand blockchain can be defined and created using third party Algorand tools, or even direct API calls on the blockchain nodes. Note that the separation of token ownership and the operational token activities is optional and advised for tokens in regulated environments where ownership should be connected to the license holder, for example electronic money tokens (EMTs) under MiCAR regulations.
